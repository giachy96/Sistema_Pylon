...
if ((((currentMillis - previousMillis)>=interval) || changeState==1) && State==Show) { // Casistica per display, se siamo in Show, metti la schermata 1 e aggiorna ogni 5 sec, o aggiorna quando il flag changestate  è attivo
    ...
    CurrentPress=millis();
      ...
      ResponseStatus rs = e22ttl.sendFixedMessage(0, 0, 5,"<232>"); // invio il codice di show
      ...
      Lastpress=millis();
      ResponseStatus rs = e22ttl.sendFixedMessage(0, 0, 5,"<332>"); //invio il codice race 
      tone(buzzer, 1000, 200);
      Serial.println("invio");//debug
    }  
    CurrentPress=millis();
      ...
      Lastpress=millis();
      ResponseStatus rs = e22ttl.sendFixedMessage(0, 0, 3,"<331>"); //invio il codice di taglio
