...
if (State==Show || State==Race){ // Ogni volta che ricevo un nuovo stato memorizzo un millis che mi serve come sincronismo (Sync) , elaboro la conferma
  if (Sync<currentMillis){ // se ancora non è arrivato il momento di trasmettere assegno il flag Transm a zero
  Transm=0;
  }
  else if (Sync>=currentMillis && (currentMillis-Sync)<= WindowTransm){ // Se è arrivato il momento di trasmettere, ovvero ho appena superato il tempo Sync e sono nella finestra temporale da Sync a Sync+150ms
    Transm=1; // Inserisco il flag di trasmissione
    Sync=Sync+Freq; // incremento il valore di Sync aggiungendo la frequenza
    }
  }
  else if (Sync>=currentMillis && (currentMillis-Sync)> WindowTransm){ // Se è arrivato il momento di trasmettere ma non siamo più nella finestra temporale di sicurezza ovvero Sync/Sync+150
    Transm=0; // non permettere di trasmettere
    while (Sync>currentMillis) { //Incrementa il valore di Sync con la frequenza finchè Sync non è di nuovo maggiore di currentmillis (sicurezza forse inutile se il codice impunta previene la trasmissione sovrapposta)
    Sync=Sync+Freq;
    }
  }
...
if ((((currentMillis - previousMillis)>=interval) || changeState==1) && State==Show) { // Casistica per display, se siamo in Show metti la schermata 1 e aggiorna ogni 5 sec, o aggiorna quando il flag changestate è attivo
    ...
    if (e22ttl.available()>1){ 
      ...
      if (rc.status.code!=1){ // If there is some problem
        ...
        Serial.print(State); // ricevo il nuovo stato
    ...
    if (Press==1 && Transm==1 && State==Show){ //se sono in Show, se ho il flag della pressione di show e se ho consenso alla trasmissione con Transm
    ResponseStatus rs = e22ttl.sendFixedMessage(0, 0, 3,"<211a>"); //mando il messaggio
    ...
    if (Pressr==1 && Transm==1 && State==Race){//se sono in Race, se ho il flag della pressione di Race e se ho consenso alla trasmissione con Transm
    ResponseStatus rs = e22ttl.sendFixedMessage(0, 0, 3,"<311a>");//mando il messaggio
