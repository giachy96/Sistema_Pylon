...
void loop() { 
  ...
  if (TxData !="0" && (State==Show || State==Race)){ //se è arrivato qualcosa dal telecomando , e siamo nelle condizioni di gara o 
      ...
      Plotmillis=millis();
      if (TxData != OTxData && Ver==0){
      Serial.print(Plotmillis);
      Serial.println(" ");
      Serial.println(TxData);
      }
      else if(TxData==OTxData){
      Ver=1;
