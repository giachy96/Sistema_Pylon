...
String system_state; // usato per inibire il pulsante avanti/indietro quando si è in gara
bool flagcount;
bool showcroverde;
bool showcrorosso;
bool showcroblu;
...
void loop() {
  ...
  if (digitalRead(pulsanteIndietro) == LOW && currentMillis - oldPress >= intervalPress && (system_state.indexOf("GO") == -1 ||  system_state.indexOf("SHOW") == -1 //prima riga if
  ||  system_state.indexOf("2510") == -1  ||  system_state.indexOf("2520") == -1  ||  system_state.indexOf("2530") == -1 ||  system_state.indexOf("4534") == -1 //seconda riga if
  ||  system_state.indexOf("4524") == -1 ||  system_state.indexOf("4514") == -1  )) {  // se pigio il pulsante Indietro. L'ho disabilitato nelle condizioni di gara dove non pigiandolo potrebbero crearsi errori
    //Serial1.println("<6000>"); // pima mando lo stop per fermare tutto
   // delay(1000);
    ResponseStatus rs = e22ttl.sendFixedMessage(Key, Add, Chan, "700");
    oldPress = millis();
  }
 if (digitalRead(pulsanteAvanti) == LOW && currentMillis - oldPress >= intervalPress && (system_state.indexOf("GO") == -1 ||  system_state.indexOf("SHOW") == -1 //prima riga if
  ||  system_state.indexOf("2510") == -1  ||  system_state.indexOf("2520") == -1  ||  system_state.indexOf("2530") == -1 ||  system_state.indexOf("4534") == -1 //seconda riga if
  ||  system_state.indexOf("4524") == -1 ||  system_state.indexOf("4514") == -1  )) {  // se pigio il pulsante Avanti. L'ho disabilitato nelle condizioni di gara dove non pigiandolo potrebbero crearsi errori
    //Serial1.println("<6000>"); // pima mando lo stop per fermare tutto
    //delay(1000);
