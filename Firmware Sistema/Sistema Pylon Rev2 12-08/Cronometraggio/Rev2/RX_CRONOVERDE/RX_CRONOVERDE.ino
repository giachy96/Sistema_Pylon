...
void loop() {
  ...
  if(newData == true){
    ResponseStatus rs = e22ttl.sendFixedMessage(0, 1, 35, RecCh);
    newData = false;
    }
  if (e22ttl.available() > 1) { // If there is something arrived from lora (Transmitter)
    ...
    if (TxData != "0" && (State == "2000"|| State == "3000"|| State == "3523" )) { //Condition for sent to mega press
      if (AltSerial.available() > 0) { // If mega will sent state to sem_rx
        recvWithStartEndMarkers( RecCh );
        State = RecCh;
      }
      if (State == "2000" || State == "3000"|| State == "3523") { //if nothing has change from previous if-statements
